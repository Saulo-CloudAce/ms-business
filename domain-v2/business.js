const {
  hasFieldUnique,
  hasCustomerFields,
} = require("../src/lib/template-validator");
...
class Business {
  ...
  async create(
    ...
    const { invalids, valids, validsCustomer } =
      await this.validator.validateAndFormat(
        file.path,
        fields,
        jumpFirstLine,
        dataSeparator,
    ...
    if (hasCustomerFields(fields)) {
      ...
      this.crmService
        ...
        .catch(() => {
          console.log(
            "Errro ao enviar customers para CRM - BUSINESS_ID:",
            businessId
          );
        });
    }
  ...
  async createFromUrlFile(
    ...
    const { invalids, valids, validsCustomer } =
      await this.validator.validateAndFormatFromUrlFile(
        filepath,
        fields,
        jumpFirstLine,
        dataSeparator,
  ...
  async createFromJson(
    ...
    const { invalids, valids, validsCustomer } =
      await this.validator.validateAndFormatFromJson(data, fields, listBatches);
    ...
    const filePath = await this.uploader.uploadContent(
      companyToken,
      uploadContentRequestBody,
      filename
    );
    const businessId = await this.repository.save(
      companyToken,
  ...
  async createSingleFromJson(
    ...
    if (hasCustomerFields(fields)) {
      ...
      if (responseSendData.data && responseSendData.data.contact_ids)
        contactIdList = responseSendData.data.contact_ids;
    }
  ...
  async listAllBatchesAndChildsByTemplateId(companyToken, templateId) {
    try {
      return this.repository.listAllBatchesAndChildsByTemplate(
        companyToken,
        templateId
      );
    } catch (err) {
  ...
  async getByNameAndTemplateId(companyToken, businessName, templateId) {
    try {
      return this.repository.getByNameAndTemplateId(
        companyToken,
        businessName,
        templateId
      );
  ...
  async getAllBatchesBasicPaginated(companyToken, page = 0, limit = 10) {
    try {
      return this.repository.getAllBatchesBasicPaginated(
        companyToken,
        page,
        limit
      );
  ...
  async getActivatedBatchesBasicPaginated(companyToken, page = 0, limit = 10) {
    try {
      return this.repository.getActivatedBatchesBasicPaginated(
        companyToken,
        page,
        limit
      );
  ...
  async getInactivatedBatchesBasicPaginated(
    ...
    try {
      return this.repository.getInactivatedBatchesBasicPaginated(
        companyToken,
        page,
        limit
      );
  ...
  async getRegisterById(companyToken, businessId, registerId) {
    try {
      return this.repository.getRegisterById(
        companyToken,
        businessId,
        registerId
      );
  ...
  async getDataByIdPaginated(companyToken, id, page = 0, limit = 10) {
    try {
      return this.repository.getDataByIdPaginated(
        companyToken,
        id,
        page,
        limit
  ...
  async listMailingByTemplateListAndKeySortedReverse(
    companyToken = "",
    customer = {},
    templateRepository = {}
  ) {
    ...
    try {
          ...
          if (template) {
            ...
            if (fieldKey) {
              ...
              if (fieldKey.data === "customer_cpfcnpj") {
                keyValue = customer.cpfcnpj
                  ? customer.cpfcnpj
                  : customer.customer_cpfcnpj;
              } else if (
                fieldKey.data === "customer_phone" ||
                fieldKey.data === "customer_phone_number"
              ) {
                keyValue = customer.phone
                  ? customer.phone[0].number
                  : customer.customer_phone[0].customer_phone_number;
              } else if (
                fieldKey.data === "customer_email" ||
                fieldKey.data === "customer_email_address"
              ) {
                keyValue = customer.email
                  ? customer.email[0].email
                  : customer.customer_email[0].customer_email;
              } else if (fieldKey.data === "customer_name") {
                keyValue = customer.name
                  ? customer.name
      ...
      if (templateIdList.length && matchParams.length) {
        const customerMailings =
          await this.repository.listAllByTemplateListAndKeySortedReverse(
            companyToken,
            templateIdList,
            matchParams
          );
      ...
      console.error(err);
      throw Error(
        "Ocorreu erro ao listar os mailings que tenha este cliente presente"
      );
    }
  }

  async getLastMailingByTemplateListAndKeySortedReverse(
    companyToken = "",
    customer = {},
    templateRepository = {}
  ) {
    ...
    try {
      ...
      if (templateIdList.length && matchParams.length) {
        ...
        if (
          customerMailings &&
          customerMailings.length &&
          customerMailings[0].templateId
        ) {
