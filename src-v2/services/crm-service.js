...
async function sendData(
  ...
  // try {
  const result = await getAxiosInstance(companyToken).post(
    `${process.env.CRM_URL}/customers`,
    payload
  );
  return result;
  // } catch (err) {
...
async function updateCustomer(customerId, data, companyToken) {
  try {
    return await getAxiosInstance(companyToken).put(
      `${process.env.CRM_URL}/customers/${customerId}`,
      data
    );
  } catch (err) {
...
async function createSingleCustomer(data, companyToken, prefixIndexElastic) {
  try {
    ...
    return await getAxiosInstance(companyToken).post(
      `${process.env.CRM_URL}/customer`,
      data
    );
  } catch (err) {
...
async function getByCpfCnpj(cpfcnpj, companyToken) {
  try {
    return await getAxiosInstance(companyToken).get(
      `${process.env.CRM_URL}/customers?cpfcnpj=${cpfcnpj}`
    );
  } catch (err) {
    return err;
  }
...
async function getListCustomersByCpfCnpj(cpfcnpjList = [], companyToken = "") {
  try {
    return await getAxiosInstance(companyToken).post(
      `${process.env.CRM_URL}/pool_customers`,
      { cpfcnpj_list: cpfcnpjList }
    );
  } catch (err) {
    return err;
  }
...
async function getCustomerById(id, companyToken) {
  try {
    return await getAxiosInstance(companyToken).get(
      `${process.env.CRM_URL}/customers/${id}`
    );
  } catch (err) {
    return err;
  }
...
async function getCustomerFormattedById(id, companyToken) {
  try {
    return await getAxiosInstance(companyToken).get(
      `${process.env.CRM_URL}/customers/${id}/formatted`
    );
  } catch (err) {
    return err;
  }
...
async function searchCustomer(
  ...
  try {
    ...
    return await getAxiosInstanceByCompanyElastic(
      companyToken,
      prefixIndexElastic
    ).get(url);
  } catch (err) {
...
async function searchCustomerFormatted(
  search,
  companyToken,
  prefixIndexElastic,
  templateId = "",
...
async function getAllCustomersByCompany(companyToken) {
  try {
    return await getAxiosInstance(companyToken).get(
      `${process.env.CRM_URL}/customers/all`
    );
  } catch (err) {
    return err;
  }
...
async function getAllCustomersByCompanyPaginated(
  companyToken,
  page = 0,
  limit = 0,
  templateId = ""
  ...
  try {
    if (templateId && templateId.length)
      return await getAxiosInstanceByCompanyTemplateID(
        companyToken,
        templateId
      ).get(`${process.env.CRM_URL}/customers/all?page=${page}&limit=${limit}`);
    return await getAxiosInstance(companyToken).get(
