...
class BusinessController {
  ...
  async createFromUrlFile(req, res) {
    ...
    if (
      ...
      !Object.values(AggregateModeType).includes(req.body.aggregate_mode)
    ) {
      return res.status(400).send({
        errors: `O aggregate_mode pode ser ${Object.values(
          AggregateModeType
        ).join(",")}`,
      });
    }
    ...
    try {
      ...
      if (!mongoIdIsValid(templateId))
        return res.status(400).send({ error: "O ID do template é inválido" });

      if (
        moment(activeUntil, "YYYY-MM-DD").format("YYYY-MM-DD") !== activeUntil
      )
        return res.status(400).send({
          error:
            "A data active_until está com formato inválido. O formato válido é YYYY-MM-DD",
        });
      if (
        moment(activeUntil, "YYYY-MM-DD").diff(moment().format("YYYY-MM-DD")) <
        0
      )
        return res.status(400).send({
          error:
            "A data active_until não pode ser anterior a data de hoje, somente igual ou posterior",
        });
      ...
      if (!company)
        return res
          .status(400)
          .send({ error: "#00010 - Company não identificada." });
      ...
      if (!template)
        return res
          .status(400)
          .send({ error: "#00011 - Template não identificado" });
      if (!template.active)
        return res.status(400).send({
          error:
            "#00012 - Este template foi desativado e não recebe mais dados.",
        });

      const businessList = await newBusiness.getByNameAndTemplateId(
        companyToken,
        req.body.name,
        templateId
      );
      if (businessList.length)
        return res
          .status(400)
          .send({ error: `${req.body.name} já foi cadastrado` });

      const jumpFirstLine = req.body.jump_first_line
        ? req.body.jump_first_line
        : false;

      let dataSeparator = ";";
      if (req.body.data_separator) {
        if (req.body.data_separator === "," || req.body.data_separator === "v")
          dataSeparator = ",";
      ...
      if (invalids.length)
        return res.status(400).send({ businessId, invalids });
  ...
  async create(req, res) {
    ...
    try {
      ...
      if (businessList.length)
        return res
          .status(400)
          .send({ error: `${businessName} já foi cadastrado` });

      const jumpFirstLine = req.body.jump_first_line
        ? req.body.jump_first_line.toLowerCase() === "true"
        : false;
  ...
  async createSingleRegisterBusiness(req, res) {
    ...
    if (data.length > 1)
      return res
        .status(400)
        .send({ error: "É possível cadastrar apenas um registro." });
    ...
    try {
      ...
      if (moment(activeUntil).diff(moment()) < 0)
        return res.status(400).send({
          error:
            "A data active_until não pode ser anterior a data de hoje, somente posterior",
        });
      ...
      const businessName = `single-register-${moment().format(
        "YYYYMMDDHmmss"
      )}${rndNumber}`;

      const isBatch = false;

      const { businessId, invalids, contactIds, valids } =
        await newBusiness.createSingleFromJson(
          companyToken,
          businessName,
          template.fields,
          templateId,
      ...
      if (businessId === null)
        return res.status(400).send({ error: dataInvalids });

      if (dataInvalids.length)
        return res
          .status(400)
          .send({ businessId, isBatch, invalids: dataInvalids });
      ...
      return res
        .status(201)
        .send({ businessId, isBatch, customerId: contactIds, crmIds });
    } catch (e) {
      console.error("CREATE BUSINESS FROM JSON ==> ", e);
      return res.status(500).send({ error: e.message });
    }
  ...
  async getPoolData(req, res) {
    ...
    if (!companyToken)
      return res
        .status(400)
        .send({ error: "O token da company é obrigatório." });

    const searchData = req.body.data;
    if (!Array.isArray(searchData))
      return res.status(400).send({ error: "O data é um array obrigatório." });
    else if (searchData.length === 0)
      return res
        .status(400)
        .send({ error: "O data deve ter no mínimo um item." });
    ...
    if (!req.body["fields"] || req.body.fields.length === 0)
      ...
      return res
        .status(400)
        .send({ error: "O fields é um array de preenchimento obrigatório." });

    try {
      const { companyRepository, businessRepository, templateRepository } =
        this._getInstanceRepositories(req.app);
      ...
      if (searchDataInvalid)
        return res
          .status(400)
          .send({ error: "Os itens no campo 'data' não estão corretos." });

      if (req.body.fields.indexOf("*") >= 0) {
        const listTemplateId = searchData.map((sd) => sd["schama"]);
        const templateList = await templateRepository.getByListId(
          listTemplateId,
          companyToken
        );
        ...
        for (const i in templateList) {
          ...
          const vfieldList = template.fields
            .filter((f) => f.visible)
            .map((f) => f.column);
      ...
      const businessData = await businessRepository.getDataByListId(
        companyToken,
        searchData,
        fields
      );
  ...
  async getAll(req, res) {
    ...
    try {
      ...
      const { businessList, pagination } =
        await newBusiness.getAllBatchesBasicPaginated(companyToken, 0, 10);
  ...
  async getAllActivated(req, res) {
    ...
    try {
      ...
      const businessList = await newBusiness.getActivatedBatchesBasic(
        companyToken
      );
      const business = businessList.map((b) => {
        return {
  ...
  async getAllPaginated(req, res) {
    ...
    try {
      ...
      const { businessList, pagination } =
        await newBusiness.getAllBatchesBasicPaginated(
          companyToken,
          page,
          limit
        );
  ...
  async getAllActivatedPaginated(req, res) {
    ...
    try {
      ...
      const { businessList, pagination } =
        await newBusiness.getActivatedBatchesBasicPaginated(
          companyToken,
          page,
          limit
        );
  ...
  async getAllInactivatedPaginated(req, res) {
    ...
    try {
      ...
      const { businessList, pagination } =
        await newBusiness.getInactivatedBatchesBasicPaginated(
          companyToken,
          page,
          limit
        );
  ...
  async markBusinessFlowPassed(req, res) {
    ...
    if (!mongoIdIsValid(businessId))
      return res.status(500).send({ error: "Código do lote inválido" });
    ...
    try {
      const { companyRepository, businessRepository } =
        this._getInstanceRepositories(req.app);
      ...
      if (!business)
        return res.status(400).send({ error: "Business não identificado" });

      await businessRepository.markFlowPassed(
        companyToken,
        businessId,
        updatedBy
      );
  ...
  async unmarkBusinessFlowPassed(req, res) {
    ...
    try {
      ...
      await businessRepository.unmarkFlowPassed(
        companyToken,
        businessId,
        updatedBy
      );
  ...
  async activateBusiness(req, res) {
    ...
    try {
      ...
      await businessRepository.activate(
        companyToken,
        businessId,
        activeUntil,
        updatedBy
  ...
  async searchDataInBusiness(req, res) {
    ...
    if (!templateId)
      return res.status(400).send({ error: "Informe o ID do template" });
    ...
    try {
      ...
      const keyColumnList = template.fields
        .filter((f) => allowedColumnType.includes(f.type))
        .map((f) => f.column);
      ...
      const templateData =
        await businessRepository.listAllAndChildsByTemplateAndKeySortedReverse(
          companyToken,
          templateId,
          keyColumnList,
      ...
      return res
        .status(500)
        .send({ error: "Ocorreu um erro ao pesquisar os clientes." });
    }
  }
  ...
  async getByIdWithDataPaginated(req, res) {
    ...
    const businessId = req.params.id;
    if (!businessId)
      return res.status(400).send({ error: "O ID do business é obrigatório." });
    ...
    try {
      ...
      const business = await newBusiness.getDataByIdPaginated(
        companyToken,
        businessId,
        page,
        limit
      ...
      return res
        .status(500)
        .send({ error: "Ocorreu erro ao listar os dados paginados" });
    }
  }
  ...
  async deactivateExpiredBusiness() {
    ...
    try {
      ...
      const businessList = await businessRepository.getExpiredBusiness(
        currentDate
      );
        ...
        businessRepository
          ...
          .then(() => {
            console.log(
              ...
              "on company",
              companyToken,
              "disabled by expiration"
            );
          })
          .catch((err) => {
            console.error(
              "Error on disable mailing by expiration. Mailing",
              businessId,
              "on company",
              companyToken
            );
  ...
  async updateBusinessRegisterById(req, res) {
    ...
    if (!templateId)
      return res
        .status(400)
        .send({ error: "Informar o ID do tempolate no header" });

    const registerId = req.params.registerId;
    if (!registerId)
      return res
        .status(400)
        .send({ error: "Informar o ID do registro que deseja alterar." });

    const businessId = req.params.businessId;
    if (!businessId)
      return res
        .status(400)
        .send({ error: "Informar o ID do lote que deseja alterar." });
    ...
    try {
      ...
      if (fieldEditableList.length === 0)
        return res
          .status(400)
          .send({ error: "Este template não possui campos editáveis" });

      const businessList =
        await businessRepository.getDataByIdAndChildReference(
          companyToken,
          businessId
        );
      if (!businessList)
      ...
      const register = await businessRepository.getRegisterByBusinessAndId(
        companyToken,
        businessId,
        registerId
      );
      ...
      fieldEditableList.forEach((f) => {
        if (dataUpdate[f.column] && String(dataUpdate[f.column]).length > 0) {
          if (
            (f.type === "array" || f.type === "options") &&
            !Array.isArray(dataUpdate[f.column])
          ) {
      ...
      template.fields.forEach((data) => {
        Object.keys(register).forEach((keysRegister) => {
          if (keysRegister === data.column)
            objCRM[data.data] = JSON.parse(
              JSON.stringify(register[data.column])
            );
        });
      ...
      const searchCustomerCRM = await crmService.getCustomerById(
        dataUpdate.idCrm,
        companyToken
      );

      if (!searchCustomerCRM.data)
        return res
          .status(500)
          .send({ error: "Os dados não foram atualizados corretamente." });
      await crmService.updateCustomer(
        searchCustomerCRM.data.id,
        objCRM,
        companyToken
      );
      ...
      return res
        .status(500)
        .send({ error: "Ocorreu erro ao atualizar o registro" });
    }
  }
  ...
  async getBusinessRegisterById(req, res) {
    ...
    try {
      ...
      if (global.cache.business_data[cacheKey]) {
        ...
        if (
          registerCached &&
          registerCached.expire &&
          calcExpireTime(new Date(), registerCached.expire) <
      ...
      const business = await newBusiness.getRegisterById(
        companyToken,
        businessId,
        registerId
      );
  ...
  async getBusinessAndRegisterIdByCpf(req, res) {
    ...
    if (!req.query["cpfcnpj"])
      return res
        .status(400)
        .send({ error: "É obrigatório informar o CPF/CNPJ." });
    ...
    try {
      ...
      const fieldCPFCNPJ = template.fields.find(
        (f) => f.data === "customer_cpfcnpj"
      );
