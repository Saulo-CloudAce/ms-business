...
class Validator {
  ...
  async validateAndFormatFromJson(data, fields, listBatches = []) {
    ...
    data.forEach((line, i) => {
      const lineWithRulesFields = this._mapLineDataToLineDataWithRules(
        line,
        rulesByColumn
      );

      const { valid, lineErrors } = this.validate(
        lineWithRulesFields,
        i,
        listBatches,
        fields
    ...
    templateFields.forEach((field) => {
      if (field.type === "array" && Object.keys(field).includes("fields")) {
        field.fields.forEach((subField) =>
          listColumnsName.push(subField.column)
        );
      } else {
            ...
            if (lineno === firstLine) {
              ...
              if (columnsName.length < fileColumnsName.length) {
                ...
                const columnsDuplicated =
                  listElementDuplicated(fileColumnsName);
                lineInvalids.push({
                  error:
                    "O arquivo tem mais colunas do que as definidas no template",
              ...
              const jsonData = self._convertFileDataToJSONData(
                data,
                fileColumnsName,
                fields
              );
              const lineWithRulesFields = self._mapLineDataToLineDataWithRules(
                jsonData,
                rulesByColumn
              );
              const lineNumberAtFile = lineno - 1;
              const { valid, lineErrors } = self.validate(
                lineWithRulesFields,
                lineNumberAtFile,
                listBatches,
                fields
              ...
              if (valid) {
                ...
                const customerFormatted = self.formatCustomer(
                  jsonData,
                  rulesByColumn
                );
    ...
    if (columnsArray.length) {
      ...
      listLineData.forEach((line) => {
        columnsArray.forEach((col) => {
          ...
          if (lineFilled.length)
            firstLineData[col.column] =
              firstLineData[col.column].concat(lineFilled);
        });
      });
  ...
  _joinDataBatch(dataBatch, rules) {
    ...
    const columnKey =
      fieldKey.length && Object.keys(fieldKey[0]).includes("data")
        ? fieldKey[0].column
        : rules.find((r) => r.unique).column;
    ...
    dataIndexedByKeyColumn = Object.keys(dataIndexedByKeyColumn).map((k) => {
      if (dataIndexedByKeyColumn[k].length > 1)
        return this._mergeData(dataIndexedByKeyColumn[k], columnsArray);
        ...
        columnsArray.forEach((col) => {
          ...
          if (lineFilled.length)
            firstLineData[col.data] =
              firstLineData[col.data].concat(lineFilled);
        });
      });
  ...
  _joinCustomerBatch(customerBatch, rules) {
    ...
    dataIndexedByKeyData = Object.keys(dataIndexedByKeyData).map((k) => {
      if (dataIndexedByKeyData[k].length > 1)
        return this._mergeCustomer(dataIndexedByKeyData[k], columnsArray);
  ...
  async validateAndFormatFromUrlFile(
    filePath,
    fields,
    jumpFirstLine = false,
    dataSeparator = ";",
          ...
          if (String(line).length) {
            if (lineno === firstLine) {
              ...
              if (valid) {
                const dataFormatted = self.format(jsonData, rulesByColumn);
                lineValids.push(dataFormatted);
                lineValidsCustomer.push(
                  self.formatCustomer(jsonData, rulesByColumn)
                );
              } else {
                lineInvalids.push(lineErrors);
              }
  ...
  _isRequiredOrFill(rules, fieldData) {
    const dataClean = encodeURI(String(fieldData))
      .replace(new RegExp("s/\x00//g"), "")
      .replace("%00", "");
  ...
  _validateFieldUnique(rules, fieldData, errors, listBatches = []) {
    if (String(fieldData).length === 0)
      ...
      for (const batch of listBatches) {
        const dataExist = batch.data.filter(
          (d) =>
            String(d[rules.column]).toLowerCase().trim() ===
            String(fieldData).toLowerCase().trim()
        );
        if (dataExist.length) {
          dataDuplicated = true;
          break;
  ...
  _validateFieldOptions(rules, fieldData, errors) {
    ...
    if (
      rules.list_options.filter((o) => fieldDataValues[String(o.value)])
        .length !== fieldData.length
    ) {
      errors.push({
        column: rules.column,
        error:
          "O valor informado não está entre os pré-definidos na lista de opções",
        current_value: fieldData,
  ...
  _validateFieldBoolean(rules, fieldData, errors) {
    if (
      !(
        String(fieldData).toLowerCase() === "true" ||
        String(fieldData).toLowerCase() === "false"
      )
    ) {
      errors.push({
        column: rules.column,
    ...
    if (typeof fieldData === "string") {
      ...
      if (isNaN(elText)) {
        ...
        errors.push({
          column: rules.column,
          error:
            "O telefone informado não tem a quantidade mínima de 10 números",
          current_value: fieldData,
        });
        ...
        errors.push({
          column: rules.column,
          error:
            "O telefone informado não é válido, tem todos números zerados.",
          current_value: fieldData,
        });
      }
    } else {
  ...
  _validateFieldArray(rules, fieldData, errors) {
    if (!Array.isArray(fieldData) && rules.required) {
      errors.push({
        column: rules.column,
        error:
          "Este campo é um array e é obrigatório, logo precisa ser preenchido",
        current_value: fieldData,
      });
      ...
      Object.keys(rules).includes("fields") &&
      !isArrayObject(fieldData)
    ) {
      errors.push({
        column: rules.column,
  ...
  _validateFieldCpfCnpj(rules, fieldData, errors) {
    ...
    if (elText.length !== 11 && elText.length !== 14) {
      errors.push({
        column: rules.column,
        error:
          "O valor informado não tem a quantidade de caracteres válidos para um CPF ou CNPJ",
        current_value: fieldData,
      });
    }
  ...
  validate(data, lineNumber, listBatches = [], fields = []) {
    ...
    const fieldsWithoutRules = Object.keys(data).filter(
      (k) => typeof data[k].rules !== "object"
    );

    if (fieldsWithoutRules.length) {
      const listFieldsRequired = fields
        .filter((f) => fieldsWithoutRules[f.data])
        .map((f) => f.column);
      const fieldsColumnsTemplate = fields.map((f) => f.column);
      const fieldsColumnsReceived = Object.keys(data);
      const fieldsDiff = arraysDiff(
        fieldsColumnsTemplate,
        fieldsColumnsReceived
      );
    ...
    Object.keys(data).forEach((k) => {
      ...
      if (isRequired(rules)) {
        lineErrors.errors = this._validateFieldRequired(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isKey(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldKey(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isUnique(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldUnique(
          rules,
          el,
          lineErrors.errors,
          listBatches
      ...
      if (isTypeDate(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldDate(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeInt(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldInt(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeOptions(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldOptions(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeDecimal(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldDecimal(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeCep(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldCep(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeBoolean(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldBoolean(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeEmail(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldEmail(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypePhoneNumber(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldPhoneNumber(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeArray(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldArray(
          rules,
          el,
          lineErrors.errors
        );
      ...
      if (isTypeCpfCnpj(rules) && this._isRequiredOrFill(rules, el)) {
        lineErrors.errors = this._validateFieldCpfCnpj(
          rules,
          el,
          lineErrors.errors
        );
  ...
  _formatFieldArray(fieldRules, fieldData) {
    ...
    if (!fieldRules["fields"]) {
      ...
      if (Array.isArray(fieldData)) {
        const isArrayPhone =
          fieldRules.fields.filter((f) => f.type === "phone_number").length > 0;
        fieldData
          .filter(
            (fd) =>
              Object.keys(fd).filter((fdk) => String(fd[fdk]).length > 0)
                .length > 0
          )
          .forEach((element) => {
            const item = {};
            ...
            if (isArrayPhone) {
              ...
              if (
                itemData["customer_phone_number"] &&
                parseInt(itemData["customer_phone_number"]) === 0
              )
                return;
              else if (
                itemData["customer_phone"] &&
                parseInt(itemData["customer_phone"]) === 0
              )
                return;
          ...
          .forEach((element) => {
            ...
            if (isArrayPhone) {
              ...
              if (
                ...
                parseInt(item["customer_phone_number"]) === 0
              )
                return;
              else if (
                item["customer_phone"] &&
                parseInt(item["customer_phone"]) === 0
              )
                return;
